...
#define REF                                                                    \
  5.08 // Modify according to actual voltage
...
#define ChannelNumber 5
      UBYTE ChannelList[ChannelNumber]
        = { 0, 1, 2, 3, 4 }; // The channel must be less than 10
      ...
      for (i = 0; i < ChannelNumber; i++)
        {
          ...
          if ((Value[i] >> 31) == 1)
            printf ("IN%d is -%lf \r\n", ChannelList[i],
                    REF * 2 - Value[i] / 2147483648.0 * REF); // 7fffffff + 1
          else
            printf ("IN%d is %lf \r\n", ChannelList[i],
                    Value[i] / 2147483647.0 * REF); // 7fffffff
        }
      for (i = 0; i < ChannelNumber; i++)
        {
          ...
          for (i = 0; i < 10; i++)
            {
              if ((ADC[i] >> 23) == 1)
                printf ("IN%d is -%lf \r\n", i,
                        REF * 2 - ADC[i] / 8388608.0 * REF); // 7fffff + 1
              else
                printf ("IN%d is %lf \r\n", i,
                        ADC[i] / 8388607.0 * REF); // 7fffff
            }
      ...
      if (isSingleChannel)
        {
          ...
          time = (double)(finish.tv_sec - start.tv_sec) * 1000.0
